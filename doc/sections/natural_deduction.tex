...
computadora? Debe ser precisa y rigurosa.
...
los \textit{cálculos de demostración} o \textit{sistemas de inferencia o
deductivos}: sistemas lógicos formales que permiten demostrar sentencias. Nos
son útiles pues pueden ser modelados como un tipo abstracto de datos, por lo que
son representables en la computadora.
...
¿Cómo podría ser formalizada en un \textit{sistema de inferencia}?
...
Tiene dos tipos de \textit{reglas de inferencia} para cada conectivo ($\wedge$,
$\vee$, $\exists$, $\dots$), que nos permite hacer dos clases de razonamiento

\begin{itemize}
    \item \textbf{Introducción}: ¿Cómo demuestro una fórmula formada por este conectivo?
    \item \textbf{Eliminación}: ¿Cómo uso una fórmula formada por este conectivo para demostrar otra?
\end{itemize}
...
\begin{definition}[Relación \diffNew{de consecuencia}]
    ...
    \diffNew{En una cantidad finita de pasos podemos, a partir de las fórmulas de $\ctx$,
    los axiomas y las reglas de inferencia, llegar a $\form$. Decimos que
    $\form$ es \textit{derivable} a partir de $\ctx$. Cuando el contexto es
    omitido, $\judG \form$ es una abreviación de $\emptyset \judG \form$. Se
    puede derivar del contexto vacío, es universalmente cierta.}
\end{definition}
...
Pero esto representó un problema para la generación de demostraciones, pues se daban conflictos en casos inofensivos: si tenemos una demostración de un teorema que usa las variables $x, y$, y lo citamos en otro que también las usa, el conflicto sería accidental, no estamos queriendo razonar de forma inválida. Pero esta regla no lo permite, por lo que al generar la demostración habría que renombrar con cuidado.

Para evitar esos problemas, la reformulamos: en lugar de validar que no esté, directamente se remueve del contexto todas las hipótesis que contengan libre esa variable en la sub-demostración. Si el conflicto era accidental, la demostración sigue chequeando (porque no se usaban las hipótesis que generaban conflicto). Pero si se usaban, el chequeador deberá reportar un error al no encontrar la hipótesis.
