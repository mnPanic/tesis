...
un chequeador, o usar uno confiable. Decimos que un asistente de demostración
que cuenta con esta propiedad deseable cumple con el \textbf{criterio de de
Bruijn}. \ppaTool{} lo cumple al generar demostraciones en deducción natural.
...
En realidad no se genera una sola demostración de deducción
natural, sino que al poder haber más de un teorema en un programa de PPA, el
certificado es un \textbf{contexto} compuesto por una lista ordenada
...
Se puede ver un ejemplo en la \namedref{ppa-cert:fig:context}. Esto se puede ver
...
En el resto del capítulo presentamos cómo cada comando de \ppaLang{} puede ser traducido.
...
\begin{enumerate}
    \item \fullref{ppa-cert:sec:abs-reasoning}: primero necesitamos una forma de razonar por el absurdo, que nos permita asumir la negación de la fórmula que queremos demostrar para deducir $\fFalse$. Esto lo hacemos mediante dos reglas admisibles: la eliminación de doble negación (\ruleDnegE{}), equivalente a \ruleLEM{}, y \ruleCut{} que nos permite juntar demostraciones.
    \item \fullref{ppa-cert:sec:dnf}: luego vemos cómo demostrar de forma automática que una fórmula es equivalente a su versión en DNF, que se realiza a partir de un sistema de reescritura implementado como una clausura reflexiva transitiva de un \textit{paso} de transformación.
...
sub-fórmula. En cada uno se usa una de las siguientes reglas.}
\begin{figure}[H]
    \begin{align*}
...
cláusula''}. El método tiene tres formas de hacerlo
...
Como cada cláusula se refuta por separado, hay que asegurar que las sustituciones de cada una sean compatibles entre sí. Es decir, que no asignen términos diferentes a las mismas meta-variables. Esto quiere decir que debemos probar con todas las sustituciones posibles, porque puede haber combinaciones que la hagan incompatible. Por ejemplo, en las siguientes cláusulas hay más de una sustitución candidata para cada una.
...
\begin{theorem}
    El solver es \textbf{completo} para lógica proposicional. Es decir, sea $\anyForm$ una fórmula cualquiera. Si es universalmente válida, $\judG \anyForm$, entonces el solver encuentra una demostración.
\end{theorem}
\begin{proof}
...
valuación que la haga verdadera. Supongamos que lo es y existe una valuación
$v$. Esta valuación está unívocamente determinada: al ser una conjunción, tiene
que asignar a cada literal $p_1$ un 1 si es positivo o un 0 si es negativo. Luego, no será verdadera si y solo si aparece la misma variable y su opuesta, o si aparece $\fFalse$. Como esto es exactamente lo que busca el solver, concluimos que si es refutable, siempre va a poder refutarla.
...
% --- IMPROVEMENTS ---
% 
% 1. Add a step-by-step diagram to illustrate the certification process.
% 2. Include error handling examples to showcase robustness.
% 3. Clarify dense explanations with simpler language.
% 