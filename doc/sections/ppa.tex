...
Por ejemplo, 
\begin{multicols}{2}
    \lstinputlisting[firstline=1, lastline=9]{listings/by-imp-then.ppa}
    \vfill\null
    \columnbreak
    \lstinputlisting[firstline=11, lastline=23]{listings/by-imp-then.ppa}
\end{multicols}
...
cases by h1, ..., hn
    ...
    \item\label{ppa:cmd:take} \lstinline{take} (\ruleExistsI) \todo{completar}
    \item\label{ppa:cmd:consider} \lstinline{consider} (\ruleExistsE) \todo{completar}
    \item \lstinline{let} (\ruleForallI) \todo{completar}
\end{itemize}
...
\lstinputlisting[caption=Descarga de conjunción simple]{listings/discharge-simple.ppa}
