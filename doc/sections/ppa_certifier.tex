...
\begin{itemize}
    ...
    \end{align*}
    \item Convertimos la fórmula a forma normal disyuntiva (DNF), disyunción de
...
natural}. Veamos un ejemplo sin generar la demostración, sin cuantificadores.
...
negación, pasando a demostrar $\fNot \fNot \form$, que al introducirla, debemos
demostrar el secuente $\ctx, \fNot \form \judG \fFalse$. Es decir, asumiendo que
no es cierta la fórmula, deducimos una contradicción, que es justo lo que
estábamos buscando.
...
Tenemos que generar una demostración de que la negación de la fórmula genera una
contradicción, pero lo queremos hacer a partir de la fórmula en DNF.
...
\begin{theorem}{(Conversión a DNF)}
    ...
    Para toda fórmula $\form$, existe $\form_{dnf}$ su equivalente en DNF.
    Podemos demostrar $\form \judG \form_{dnf}$
    \todo{SeGUIR}
\end{theorem}
...
\end{theorem}
\begin{proof}
    La demostración es trivial. Basta con caminar recursivamente el $\fAnd$ y
    usar \ruleAndEOne{} si el camino para encontrar a $\anyForm_i$ sigue por la
    izquierda y \ruleAndETwo{} si sigue por la derecha.
\end{proof}
