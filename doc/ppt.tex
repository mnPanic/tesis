...
\begin{frame}{Representación de demostraciones}
    ...
    \begin{itemize}
        \item Tenemos dos axiomas
        \begin{enumerate}
            \item Los alumnos que faltan a los exámenes, los reprueban.
            \item Si se reprueba un final, se recursa la materia.
        \end{enumerate}
        \item A partir de ellos, podríamos demostrar que si un alumno falta a un final, entonces recursa la materia.
    \end{itemize}
...
\begin{frame}{Ejemplo}
    \begin{example}[Demostración en DN]
        ...
        \RL{\uncover<6->{\ruleAx}}
        \only<-5>{\noLine}
        \UnaryInfC{\uncover<6->{$\ctx \judG (\falta \fImp \reprueba) \wedge (\reprueba \fImp \recursa)$}}
        \RL{\uncover<6->{\alert<6>{\ruleAndETwo}}}
        \only<-5>{\noLine}
        \UnaryInfC{\uncover<5->{$\ctx \judG \reprueba \fImp \recursa$}}
    
        \AxiomC{\uncover<7->{\alert{$\someProof$}}}
        \noLine
        \UnaryInfC{\uncover<5->{$\ctx \judG \reprueba$}}
...
\begin{frame}{Certificado del by}
    ...
    \begin{enumerate}[<+->]
        ...
        \[
            \ctx \judG \formTwo_1 \fAnd \dots \fAnd \formTwo_n \fImp \form
        \]
        \item \textbf{Razonamos por el absurdo}: Asumiendo la negación buscamos una contradicción
        \[
            \ctx, \fNot (\formTwo_1 \fAnd \dots \fAnd \formTwo_n \fImp \form) \judG \fFalse
        \]
        \item Convertimos la negación a forma normal disyuntiva (\textbf{DNF})
        \[
            \ctx, (\formLit_{1} \fAnd \dots \fAnd \formLit_n)
            \fOr \dots \fOr
            (\formLitTwo_{1} \fAnd \dots \fAnd \formLitTwo_m) \judG \fFalse
        \]
        ...
        \begin{itemize}
            ...
            \item Eliminando universales consecutivos y reiniciando el proceso, se consigue una refutación ($\fNot \pred(k), \forall \var . \pred(\var)$)
        \end{itemize}
    \end{enumerate}
\end{frame}

\begin{frame}{Ejemplo sin cuantificadores (1/3)}
    \begin{figure}
        \centering
...
\begin{frame}{Razonamiento por el absurdo}
    \begin{exampleblock}{Razonamiento por el absurdo en DNF}
        \[
            \ctx \judG \big((a \fImp b) \wedge a \big) \fImp b
            \quad\overset{?}{\rightsquigarrow}\quad
            \ctx, \fNot [ \big( (a \to b) \fAnd a \big) \to b ] \judG \fFalse
        \]
    \end{exampleblock}
    \pause
    ...
    \begin{lemma}[Razonamiento por el absurdo]
        \begin{columns}
            \begin{column}{0.5\textwidth}
                \begin{prooftree}
                    \AxiomC{$\ctx, \fNot \form \judG \fFalse$}
                    \RL{\ruleNotI}
...
\begin{frame}{Ejemplo con cuantificadores (1/2)}
    \begin{figure}[H]
        \centering
...
\begin{frame}{Ejemplo con cuantificadores (2/2)}
    \begin{enumerate}[<+->]
        \setcounter{enumi}{2}
        \item La convertimos a DNF ($\forall$ es opaco)
        \begin{align*}
            &\fNot \left[
                \Big(\big(\forall x. (p(x) \fImp q(x))\big) \fAnd p(k) \Big)
                \fImp q(k)
        ...
        \item Eliminamos $\forall x. (p(x) \fImp q(x))$ (\ruleForallE{}). Reemplazamos
        ...
        \[
            ...
            \end{aligned}
        \]
        \item Refutamos cada cláusula (con unificación).
        \begin{itemize}
...
\begin{frame}[fragile]{Alcance y limitaciones del by}
    \begin{itemize}
        ...
        \item ¿Por qué heurístico? Elimina los $\forall$ consecutivos de a lo sumo una hipótesis (Pero le falta aún más)
    \end{itemize}
\pause
\begin{figure}
    \centering
...
\begin{frame}[fragile]{Extracción indirecta con instanciación}
    \onslide<1->{\lstinputlisting[firstline=1,lastline=3]{listings/ppt/extract/parientes.ppa}}\vspace*{-\baselineskip}
    \onslide<2->{\lstinputlisting[firstnumber=last,firstline=4,lastline=11]{listings/ppt/extract/parientes.ppa}}\vspace*{-\baselineskip}
...
\begin{frame}{Lógica intuicionista}
    ...
    \end{itemize}
    \pause
    \begin{block}{Isomorfismo Curry-Howard}
        \vspace{-\baselineskip}
        \begin{align*}
            \text{Demostraciones en DN} &\leftrightarrow \text{Términos de $\lambda$-cálculo.}\\
            \text{Normalización} &\leftrightarrow \text{Semántica (Reducciones)}
        \end{align*}
...
\onslide<5->{\textbf{Problema}: Muy lento (demostraciones muy grandes).}

\onslide<6->{
\textbf{Estrategias alternativas}: reducción en un paso o muchos paso
\begin{itemize}
...
\begin{frame}{Limitaciones}
    \begin{itemize}[<+->]
        ...
        \begin{itemize}
            \item \textit{Mejora}: Usar una \textit{máquina abstracta} que implemente reducción a forma normal.
        \end{itemize}
    \end{itemize}
\end{frame}
...
\begin{frame}{Traducción de doble negación relativizada}
    \begin{definition}[Traducción de doble negación relativizada]
        Podemos ver $\fNot \form \equiv \form \fImp \fFalse$. Definimos $\fNotR \form
        \equiv \form \rightarrow R$,
        \vspace{-\baselineskip}
        \begin{columns}
            ...
            \begin{column}{0.3\textwidth}
                \begin{itemize}
                    \item Método general para embeber clásica en intuicionista.
                    \item En clásica son equivalentes (\ruleDnegE{}).
                    \item En intuicionista es más débil.
                \end{itemize}
            \end{column}
        \end{columns}
    \end{definition}
    ...
    \begin{theorem}
        ...
        $\tdn{\someProof} \proves \tdn{\ctx} \judgI \alert<3>{\transDNeg{\form}}$
    \end{theorem}
\end{frame}
...
\begin{frame}{Introducción de negación relativizada}
    ...
    \pause
    \begin{proof}
        Por inducción estructural en la fórmula.
        \begin{itemize}
            \item Atómicas trivial (y eso aparece en la bib)
...
\begin{frame}{Traducción de demostraciones}
    ...
    \begin{proof}
        ...
        \textbf{Estrategia}: traducir recursivamente las partes de $\someProof$ y usarlas para construir una nueva demostración de $\tdn{\form}$.
    \end{proof}
\end{frame}
        ...
        \begin{columns}
            \begin{column}{0.5\textwidth}
                \begin{prooftree}
                    \AxiomC{$\someProof_\form$}
                    \noLine
                    \UnaryInfC{$\ctx \judgC \form$}
                    \AxiomC{$\someProof_\formTwo$}
                    \noLine
                    \UnaryInfC{$\ctx \judgC \formTwo$}
                    \RL{\ruleAndI}
                    \BinaryInfC{$\ctx \judgC \form \wedge \formTwo$}
                \end{prooftree}        
            \end{column}
            \begin{column}{0.5\textwidth}
...
\begin{frame}{Traducción de demostraciones}
    ...
    \begin{proof}
        ...
        \begin{itemize}[<+->]
            ...
            \ruleOrIOne{}, \ruleOrITwo{}, \ruleForallI{}, \ruleForallE{}, \ruleNotI{}, \ruleNotE{}, \ruleTrueI{}, \ruleAx{}, \ruleExistsI{} directas.
            \item \alert{\ruleLEM{}} interesante.
            \item \ruleFalseE{} inducción estructural sobre la fórmula.
            \item \ruleOrE{} y \ruleExistsE{} análogos. Truco: No vale \ruleDnegE{} pero si \ruleDnegRE{} (probado por inducción estructural sobre la fórmula).
        \end{itemize}
    \end{proof}
\end{frame}
...
\begin{frame}{Traducción de Friedman}
    ...
    \]
    \begin{itemize}[<+->]
        \item Traducción de doble negación (fórmulas + demostraciones)
        \[
            \tdn{\someProof} \proves \tdn{\ctx} \judgI \alert{\tdn{\anyFormTwo}}.
        \]
        \item Traducción de Friedman, manteniendo fórmula (deben ser $\classPiTwo$ con $\anyForm$ \textbf{conjuntiva})
        \[
            ...
            \anyForm(\var, \varTwo_1, \dots, \varTwo_n).
        \]
        \item Manteniendo contexto (axiomas deben ser \textbf{F-fórmulas})
        \[
