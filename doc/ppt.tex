...
\begin{frame}{Asistentes de demostración}
    ...
    \todo{No me gusta la palabra teoria, lo usamos para teorias de primer orden}
\end{frame}
...
\begin{frame}{PPA}
    ...
    \begin{itemize}
        ...
        \item \textbf{Extracción de testigos}: De una demo de
        $\exists \var . \pred(\var)$, extrae $\term$ tq $\pred(\term)$.
    \end{itemize}
\end{frame}

\begin{frame}{Aporte principal}
    \centering
    \Large
    Implementación de extracción de testigos para lógica clásica de forma \textit{indirecta}.
\end{frame}
...
\begin{frame}{Sistemas deductivos}
    \begin{itemize}
        ...
        \item Necesitamos \textbf{sistemas deductivos}: sistemas lógicos formales usados para escribir demostraciones de setencias.
        \item En particular usamos \textbf{deducción natural}. Compuesto por,
        \begin{itemize}
...
\begin{frame}{Reglas admisibles}
    \begin{itemize}
        ...
        \item Se implementan como funciones o macros.
    \end{itemize} 
...
\begin{frame}{Mathematical vernacular}
    Hay una forma canónica de representar demostraciones matemáticas\footnote{\textit{Mathematical
...
\begin{frame}{Demostraciones}
    \begin{columns}
        ...
        \begin{column}{0.5\textwidth}
            \begin{itemize}
                \item Lista de \textbf{comandos} que reducen sucesivamente la \textit{tesis}
                ...
                asumidas (axiomas) o demostradas (teoremas y comandos que
...
\begin{frame}{En general}
    \centering
    \large
    Dada una demostración de $\exists \var . \pred(\var)$, extraer un término $\term$ que sea \textit{testigo del existencial}, i.e. que cumpla $\pred(\term)$.
\end{frame}
...
\begin{frame}{Extracción con instanciación}
    \lstinputlisting[title=Extracción con instanciación]{listings/extract/forall.ppa}

    Extraemos un testigo \lstinline{v} y podemos instanciar \lstinline{X} en lo que sea, por ej. la siguiente fórmula es demostrable: \lstinline{p(f(n, m), v)}
\end{frame}

\begin{frame}{Extracción indirecta}
    \lstinputlisting[firstline=2, title=Extracción indirecta]{listings/extract/indirect.ppa}
    Extracción \textbf{indirecta} de \lstinline{theorem t2} nos da el testigo \lstinline{m}.
\end{frame}

\begin{frame}{Extracción por el absurdo (1/2)}
    \lstinputlisting[
...
\begin{frame}{Extracción por el absurdo (2/2)}
    \begin{columns}
        ...
        \end{column}
        \begin{column}{0.3\textwidth}
            Sin \lstinline{take} (\ruleExistsI) explícito, igual podemos extraer el testigo a partir del \lstinline{theorem} \lstinline{hayAlguienBajo}: \lstinline{juan}.
        \end{column}
    \end{columns}
\end{frame}

\begin{frame}{Extracción de testigos}
...
\begin{enumerate}
    \item la certifique generando una demostración en deducción natural \textbf{clásica}
...
\begin{frame}{Lógica clásica}
    \centering
    La lógica clásica \textbf{no es constructiva} por LEM
    \vspace{0.5cm}
    \proofTreeLEM
\end{frame}
...
\begin{frame}{Demostración no constructiva}
    \begin{ejemplo}[Fórmula sin demostración constructiva]
        Sea $C$ algo indecidible (tipo \texttt{HALT}), queremos ver que
        \[
            \exists y .
                (y = 1 \fAnd \alert{C})
                \fOr
    ...
    Existen fórmulas que admiten demostraciones constructivas y no constructivas, y otras solo no constructivas. % vale la pena lógica clásica
\end{frame}

\begin{frame}{Clases de estrategias de extracción}
    \begin{figure}
        \centering
        \includegraphics[scale=0.25]{img/ppt/extract.png}
    \end{figure}
...
\begin{frame}{Lógica intuicionista}
    ...
    \begin{itemize}
        ...
        \item Noción de \textit{forma normal} buena: una demostración de un $\exists$ debería comenzar con \ruleExistsI{}:
        \proofTreeExistsI{}
        \item Proceso de normalización análogo a reducción de $\lambda$-cálculo (su semántica operacional), visto desde el isomorfismo Curry-Howard.
    \end{itemize}
\end{frame}

\begin{frame}{Traducción de Friedman}
    \begin{itemize}
        \item La \textbf{traducción de Friedman} permite embeber la lógica clásica en la intuicionista, para \textit{algunas} fórmulas.
        \item Traduce demostraciones de fórmulas $\classPiTwo$: de la forma \[
...
\begin{frame}{Traducción de demostraciones}
    ...
    Inducción estructural sobre la demostración.

    mostrar: introducción del and, LEM, lemas
\end{frame}
...
\begin{frame}{Normalización}
    Motivación: evitar \textit{``desvíos superfluos''}.
...
\begin{frame}{Normalización de implicación}
    ...
    \begin{itemize}
        \item Primer idea: \xxcancel<2->{$\someProof_\formTwo \proves \ctx \judG \formTwo$} \pause pero ¡\textbf{no sería correcto}! La demostración $\someProof_\formTwo$ requiere la hipótesis $\hypId : \form$, que no necesariamente está en $\ctx$, es agregada por \ruleImpIh{\hypId}
        \item Correcto: usar $\someProof_\formTwo$, pero \textit{sustituyendo} todas las ocurrencias de la hipótesis $\hypId$ por la demostración $\someProof_\form$.
        \item Es necesaria una noción de sustitución de hipótesis por demostraciones (sin capturas).
    \end{itemize}
\end{frame}
...
\begin{frame}{Algoritmo de reducción}
    \begin{itemize}
        \item Original: Similar a DNF, reducir de a 1 paso sucesivamente hasta que sea irreducible.
        \item Problema: Congruencias se reducían de a un paso. Muy lento (demostraciones muy grandes)
    \end{itemize}
...
\begin{frame}{Manteniendo el contexto}
    
\end{frame}

\begin{frame}{Animación}
    Animación de la reducción
\end{frame}

\section{Detalles de implementación}

\begin{frame}{La herramienta \ppaTool{}}
    - modulos (para dar una idea de la cantidad y complejidad del código)
    - haskell
    - comandos check y extract
\end{frame}

\begin{frame}{Parser y lexer}
    diagrama para los dos caminos para hacer un parser.
    - happy y alex
\end{frame}
