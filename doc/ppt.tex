...
\begin{frame}{Teoremas}
    \begin{itemize}
        ...
        \item \textbf{Demostración} de un teorema: \textit{Argumento lógico} que usa las \textit{reglas de inferencia} de un \textit{sistema deductivo} para establecer que el teorema es una \textit{consecuencia lógica} de los \textit{axiomas} y teoremas probados anteriormente.
...
\begin{frame}{Asistentes de demostración}
    ...
    \end{figure}
    \begin{block}{Extracción de testigos}
        De una demo de
...
\begin{frame}{Sistemas deductivos}
    \begin{itemize}
        ...
        \begin{itemize}
            ...
            \item \textbf{Reglas de inferencia}: Por ejemplo, \textit{modus ponens} (si
...
\begin{frame}{Sustitución sin capturas}
    ...
    Queremos evitar la \textbf{captura de variables}, por ejemplo
    \[
...
\begin{frame}{Mathematical vernacular}
    Hay una forma natural de representar demostraciones matemáticas\footnote{\textit{Mathematical
    ...
    \begin{itemize}
        ...
        tener que demostrarlo a mano (automáticamente).
    ...
    \begin{columns}
        ...
        \begin{column}{0.5\textwidth}
            \begin{itemize}
                ...
                (fórmula a demostrar) hasta agotarla.
...
\begin{frame}{by - El mecanismo principal de demostración}
    ...
    \end{center}
    \begin{itemize}
        \item Afirma que la fórmula es una consecuencia lógica de las fórmulas que corresponden a las hipótesis provistas.
        %\item Los nombres de las hipótesis son del tipo \lstinline{<name>} (o bien identificadores o \textit{strings} arbitrarios).
        \item Demostrado \textbf{automáticamente}: Por debajo usa un \textit{solver} completo para lógica proposicional pero \textit{heurístico} para primer orden.
        \item Dos comandos principales: \lstinline{thus} y \lstinline{have}.
    \end{itemize}
\end{frame}
...
\begin{frame}{Extracción con instanciación}
    ...
    Extraemos un testigo \lstinline{cero} y podemos instanciar \lstinline{N} en lo que sea, por ej. la siguiente fórmula es demostrable: \lstinline{cero <= 60}
\end{frame}
...
\begin{frame}{Extracción por el absurdo}
    ...
    \begin{itemize}
        ...
        \item \alert{La implementación no es tan directa como buscar un \ruleExistsI}
    \end{itemize}
...
\begin{frame}{¿Entonces por qué lógica clásica?}
    \begin{itemize}
        ...
        %\item Lógica clásica permite razonar por el absurdo: \ruleDnegE{}, \ruleLEM{}
    \end{itemize}
\end{frame}
...
\begin{frame}{El truco de Friedman}
    ...
    \begin{theorem}[Traducción de Friedman]
        Sea $\someProof$ una demostración clásica de \[
            ...
            \anyForm(\var, \varTwo_1, \dots, \varTwo_n),
            \] y
            $\anyForm$ una fórmula \textbf{conjuntiva}.
...
\begin{frame}{El truco de Friedman}
    ...
    \begin{lemma}[Traducción de Friedman simplificada]
        ...
        \[
            \transDNeg{\ctx} \judgI \exists \var . \anyForm.
        \]
    \end{lemma}
\end{frame}
\begin{frame}{El truco de Friedman}
    \begin{proof}
        Aplicando la traducción tomando $R = \exists \var . \anyForm$, tenemos que
    \[
        \tdn{\big(
...
\begin{frame}{Introducción de negación relativizada}
    ...
    \begin{proof}
        ...
        \begin{itemize}
            \item Atómicas trivial. $\fNotR \form \judgI \fNotR \tdn{\form} \iff \fNotR \form \judgI \fNotR \fNotR \fNotR \form$ sale con \textit{eliminación de triple negación}.
            \item En lógica intuicionista, el $\fNot$ contiene ``poca información''. Son más difíciles las demostraciones como $\fNotR (\form \fAnd \formTwo) \judgI \fNotR \tdn{(\form \fAnd \formTwo)}$
            \item En lógica clásica requeriría LEM.
        \end{itemize}
    \end{proof}
\end{frame}
...
\begin{frame}{Normalización de implicación}
    ...
    \begin{itemize}
        ...
        \item ¡\textbf{no sería correcto}! La demostración $\someProof_\formTwo$ requiere la hipótesis $\hypId : \form$, que no necesariamente está en $\ctx$, es agregada por \ruleImpIh{\hypId}
...
\begin{frame}{F-fórmulas}
    \begin{definition}[F-fórmulas]
        $\formTwo$ es una \textit{F-fórmula} si está generada por:
        \begin{align*}
...
\begin{frame}{Manteniendo el contexto}
    \begin{itemize}
        \item Después de la traducción, se reemplaza cada axioma
        \item Los axiomas deben ser F-fórmulas.
    \end{itemize}
\end{frame}
...
\begin{frame}{Harrop}
    \begin{columns}[t]
        ...
        \begin{column}{0.4\textwidth}
            \begin{block}{Harrop}
                \begin{align*}
                    A ::=\ & \fFalse \mid \fTrue \mid \pred(\term_1, \dots, \term_n)\\
                    G ::=\ & A \\
...
\begin{frame}{La herramienta \ppaTool{}}
    \begin{figure}
        \includegraphics[scale=0.3]{img/modules.png}
    \end{figure}
