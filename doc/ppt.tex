...
\begin{frame}{Sistemas deductivos}
    \begin{itemize}
        ...
        \item Usamos el \textit{sistema deductivo} de \textbf{deducción natural}
        \item Sistema lógico formal usado para escribir demostraciones.
        \begin{itemize}
...
\begin{frame}{Ejemplo de demostración en DN}
    ...
    \begin{example}[Demostración en DN]
        ...
        \RL{\uncover<5->{\ruleAx}}
        \only<-4>{\noLine}
        \UnaryInfC{\uncover<4->{$\ctx \judG \reprueba \fImp \recursa$}}
        %
        \AxiomC{}
        \RL{\uncover<6->{\ruleAx}}
        \only<-5>{\noLine}
        \UnaryInfC{\uncover<6->{$\ctx \judG (\falta \fImp \reprueba) \wedge (\reprueba \fImp \recursa)$}}
        \RL{\uncover<6->{\ruleAndEOne}}
        \only<-5>{\noLine}
        \UnaryInfC{\uncover<6->{$\ctx \judG \falta \fImp \reprueba$}}
        \AxiomC{}
        \RL{\uncover<6->{\ruleAx}}
        \only<-5>{\noLine}
        \UnaryInfC{\uncover<6->{$\ctx \judG \falta$}}
        \RL{\uncover<6->{\ruleImpE}}
        %
        \only<-5>{\noLine}
        \BinaryInfC{\uncover<4->{$\ctx \judG \reprueba$}}
        \RL{\uncover<3->{\ruleImpE}}
        \only<-2>{\noLine}
        \BinaryInfC{\uncover<2->{\(
            \ctx = (\falta \fImp \reprueba), (\reprueba \fImp \recursa), \falta
            \judG
            \recursa 
            \)}}
...
\begin{frame}{Conclusiones - Extracción de testigos}
    ...
    \begin{itemize}
        ...
        \begin{itemize}
            \item \textit{Mejora}: Usar una \textit{máquina abstracta}.
        \end{itemize}
    \end{itemize}
\end{frame}
